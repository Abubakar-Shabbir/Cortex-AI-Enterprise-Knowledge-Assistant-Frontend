...
import { WarningCircleIcon as AlertCircle, AtIcon as AtSign, BriefcaseIcon as Briefcase, BuildingsIcon as Buildings, CaretDownIcon as CaretDown, CheckIcon as Check, CheckCircleIcon as CheckCircle2, CircleIcon as Circle, EyeIcon as Eye, EyeSlashIcon as EyeOff, GlobeIcon as Globe, LockIcon as Lock, EnvelopeSimpleIcon as Mail, UserCircleIcon as UserRound } from '@phosphor-icons/react';
...
export default function Signup() {
  ...
  return (
    <AuthLayout title="Sign up" width="max-w-[720px]">
      <div className="auth-pop-in mb-5 flex h-12 w-12 items-center justify-center rounded-xl border border-primary/15 bg-primary/10 text-primary dark:text-primary-soft">
        {accountType === 'company' ? <Buildings className="h-5 w-5" weight="bold" /> : <UserRound className="h-5 w-5" weight="bold" />}
      </div>
      <h1 className="auth-pop-in text-2xl font-bold tracking-tight text-ink dark:text-ink-dark">
        {accountType === 'company' ? 'Register your company' : 'Create your account'}
      </h1>
      <p className="mb-7 mt-1.5 text-sm text-muted dark:text-muted-dark">
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        ...
        {accountType === 'company' && !isInvitationFlow && (
          <div className="space-y-4 rounded-xl border border-line bg-surface p-4 dark:border-line-dark dark:bg-white/5">
            ...
            <div className="group">
              ...
              <div className="relative">
                <Buildings className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted transition-colors duration-150 group-focus-within:text-primary dark:text-muted-dark dark:group-focus-within:text-primary-soft" />
                <input
                  ...
                  className={`w-full rounded-lg border ${errClass('company_name')} bg-card py-2.5 pl-10 pr-3.5 text-sm text-ink placeholder:text-muted focus:border-2 focus:border-ink focus:outline-none dark:focus:border-ink-dark dark:bg-card-dark dark:text-ink-dark dark:placeholder:text-muted-dark`}
                />
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    className="auth-select w-full rounded-lg border border-line bg-card px-3.5 py-2.5 pr-9 text-sm text-ink focus:border-2 focus:border-ink focus:outline-none dark:focus:border-ink-dark dark:border-line-dark dark:bg-card-dark dark:text-ink-dark"
                  >
                    <option value="">Select…</option>
                    {orgTypes.map((t) => <option key={t.slug} value={t.slug}>{t.name}</option>)}
                  </select>
                  <CaretDown className="pointer-events-none absolute right-3.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted dark:text-muted-dark" />
                </div>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div className="group">
                ...
                <div className="relative">
                  <Briefcase className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted transition-colors duration-150 group-focus-within:text-primary dark:text-muted-dark dark:group-focus-within:text-primary-soft" />
                  <input
                    id="company_industry" value={companyIndustry} onChange={(e) => setCompanyIndustry(e.target.value)} placeholder="Software"
                    className="w-full rounded-lg border border-line bg-card py-2.5 pl-10 pr-3.5 text-sm text-ink placeholder:text-muted focus:border-2 focus:border-ink focus:outline-none dark:focus:border-ink-dark dark:border-line-dark dark:bg-card-dark dark:text-ink-dark dark:placeholder:text-muted-dark"
                  />
                </div>
              </div>
              <div className="group">
                ...
                <div className="relative">
                  <Globe className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted transition-colors duration-150 group-focus-within:text-primary dark:text-muted-dark dark:group-focus-within:text-primary-soft" />
                  <input
        ...
        <button
          ...
          className="btn-sheen flex w-full items-center justify-center gap-2 rounded-lg bg-primary py-3 text-sm font-semibold text-white shadow-softer transition-all duration-150 hover:bg-primary-dark active:scale-[0.98] disabled:opacity-70 disabled:active:scale-100"
        >
          {signup.isPending && <Spinner size={20} />}
          <span>{signup.isPending ? 'Creating account…' : 'Sign up'}</span>
        </button>

        <p className="border-t border-line pt-5 text-center text-sm text-muted dark:border-line-dark dark:text-muted-dark">
