import { WarningCircleIcon as AlertCircle, RobotIcon as Bot, StackIcon as Boxes, CaretDownIcon as ChevronDown, CpuIcon as Cpu, DatabaseIcon as Database, EyeIcon as Eye, FileTextIcon as FileText, ChatTextIcon as MessageSquareText, DotsThreeIcon as MoreHorizontal, PlugsIcon as PlugZap, SparkleIcon as Sparkles, TrashIcon as Trash2, TrendUpIcon as TrendingUp } from '@phosphor-icons/react';
  ...
  return (
    <div>
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-[repeat(auto-fit,minmax(200px,1fr))] fade-in-up">
        {has('pages.documents') && (
          <>
            <KpiCard icon={FileText} iconBg="bg-primary/10" iconColor="text-primary" label="Total Documents" value={stats.total_documents} numeric trend={kpiTrends.documents} trendLabel="vs last 7 days" chartColor="#FF385C" />
            <KpiCard icon={Boxes} iconBg="bg-warning/10" iconColor="text-warning" label="Total Chunks" value={stats.total_chunks} numeric trend={kpiTrends.chunks} trendLabel="vs last 7 days" chartColor="#D97706" />
            <KpiCard icon={Database} iconBg="bg-success/10" iconColor="text-success" label="Storage Used" value={stats.storage_used} trend={kpiTrends.storage} trendLabel="vs last 7 days" chartColor="#16A34A" />
          </>
        )}
        {has('pages.ask_ai') && (
          <KpiCard icon={MessageSquareText} iconBg="bg-info/10" iconColor="text-info" label="Queries Today" value={stats.today_queries} numeric trend={kpiTrends.queries} trendLabel="vs yesterday" chartColor="#0EA5E9" />
        )}
        {has('pages.ai_tasks') && (
