...
import { useAddPersonalAiCredits, usePersonalAiCredits, usePersonalBilling, useRequestPersonalPlan } from '../api/hooks';

export default function PersonalBilling() {
  const { data, isLoading } = usePersonalBilling();
  const { data: credits, isLoading: creditsLoading } = usePersonalAiCredits();
  const addCredits = useAddPersonalAiCredits();
  ...
  return (
    <>
      <PageHeader title="Billing" subtitle="Your personal plan, usage, and AI credits balance — separate from any company workspace you belong to." />
      ...
      <div className="mb-4 overflow-hidden rounded-2xl border border-line bg-card shadow-soft dark:border-line-dark dark:bg-card-dark">
        {data.unlimited ? (
          <div className="flex items-start gap-4 p-6">
            ...
            <div>
              <h2 className="text-base font-semibold text-ink dark:text-ink-dark">No plan selected</h2>
              <p className="mt-1 text-sm leading-relaxed text-muted dark:text-muted-dark">You currently have no usage limits. Choose a plan below to get included AI credits and features.</p>
            </div>
          </div>
        ) : (
          <>
