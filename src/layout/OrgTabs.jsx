...
//
// Each tab is only rendered when the viewer actually holds the
// org-scoped permission its page is gated on (see App.jsx's
// RequireOrgPermission wrappers, which this list is kept in sync
// with) - previously every tab showed for every role regardless of
// rank, so a plain Member saw and could click into Settings/Billing/
// Audit Logs even though the page itself would then (now) refuse
// them. Reads `my_permissions` for THIS org specifically (by slug,
// from the full organizations list), not activeOrganization - a
// member can be viewing a different organization's tabs than
// whichever workspace is active in the sidebar switcher.
//
// Deliberately NO "organizations.manage" platform-admin fallback here
// (unlike RequireOrgPermission.jsx's narrow, organization.view-only
// one) - none of these four tabs are in BYPASS_ONLY_CODENAMES, so a
// platform Admin/Super Admin viewing a company it isn't a member of
// (Admin > Companies oversight) sees zero tabs, never a route to that
// company's members/settings/billing/audit log. See
// org_permission_service.BYPASS_ONLY_CODENAMES for why.
export default function OrgTabs({ orgSlug }) {
  ...
  const can = (codename) => permissions.includes(codename);
